<x-app-layout>
    ...
    <div class="py-6 sm:py-8">
        <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('users.update', $user) }}" class="bg-white shadow-sm sm:rounded-lg p-4 sm:p-6 space-y-4">
                @csrf
                @method('PUT')
                <div>
                ...
                <label class="inline-flex items-center gap-2 text-sm">
                    <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $user->is_active)) class="rounded border-slate-300 text-teal-700 focus:ring-teal-600">
                    Active
                </label>
